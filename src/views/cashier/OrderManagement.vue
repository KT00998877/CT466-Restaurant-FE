...
const submitNewOrder = async () => {
    ...
    try {
        const response = await api.post('/cashier/orders', newOrderData.value);
        if (response.data.success) {
...
const updatePaymentStatus = async (order) => {
    ...
    try {
        ...
        const response = await api.patch(`/cashier/orders/${order.id}/payment-status`, {
...
const updateStatus = async (order) => {
    ...
    try {
        const response = await api.patch(`/cashier/orders/${order.id}/status`, {
            status: order.status
        });
        if (response.data.success) {
...
const deleteOrder = async (id) => {
    ...
    try {
        const response = await api.delete(`/cashier/orders/${id}`);
        if (response.data.success) {
